import { definePart } from './part'

/**
 * Stage（3D 展台）与 DetailPanel（右栏标本卡）的界面文案。
 *
 * 两个组件由同一次外提任务负责，共用这一片：`stage.*` 是展台工具条/
 * 加载态，`detail.*` 是标本卡的区块标题与按钮。
 */
export const PART = definePart(
  {
    'stage.canvasLabel': '{name}的可交互三维标本：拖动旋转，滚轮缩放，点击彩色圆点认识部位',
    'stage.tool.rotate': '旋转',
    'stage.tool.zoom': '放大',
    'stage.tool.focus': '聚焦',
    'stage.tool.section': '剖切',
    'stage.tool.layers': '分层',
    'stage.tool.compare': '对比',
    'stage.tool.reset': '复位',
    'stage.captionHint': '拖动旋转 · 滚轮细看 · 圆点＝观察点',
    'stage.lifeCue': '看它的 {n} 个阶段',
    'stage.lifeCueAria': '打开{name}的生活史，在展台上逐个看它的 {n} 个阶段',
    'stage.error': '这只虫的模型没能建起来：{error}',
    'stage.loading': '正在生成 {name} 的立体标本…',
    'stage.noWebgl': '这台设备开不了 3D 视图（WebGL 不可用），先用剪影代替。图鉴的文字内容不受影响。',
    'stage.glLost': '3D 视图中断了（图形上下文丢失）。通常会自己恢复；一直没动静就重新加载。',
    'stage.reload': '重新加载',
    'stage.part.head': '头部',
    'stage.part.eye': '复眼',
    'stage.part.antenna': '触角',
    'stage.part.mouthparts': '口器',
    'stage.part.horn': '角',
    'stage.part.thorax': '胸部',
    'stage.part.wing': '翅',
    'stage.part.leg': '足',
    'stage.part.abdomen': '腹部',
    'stage.part.sound': '发声与听器',
    'stage.partJump': '看别的虫的{part} →',
    'detail.eyebrow': '本期标本',
    'detail.facts': '关键数据',
    'detail.ecology': '生态角色',
    'detail.trivia': '你知道吗',
    'detail.readGuide': '读它的图鉴详解',
    'detail.motionDemo': '动态演示',
    'detail.quiz': '小测',
    'detail.compareOthers': '与其他昆虫对比',
    'detail.share': '分享这只虫',
    'detail.copyLink': '复制本页链接',
    'detail.linkCopied': '已复制',
    'detail.range': '分布',
    'detail.status': '现状',
    'detail.relatives': '近缘',
    'detail.photos': '实物照片',
    'detail.photosLink': '在 iNaturalist 上看实物图',
  },
  {
    'stage.canvasLabel':
      'Interactive 3D specimen of {name}: drag to rotate, scroll to zoom, click the colored dots to explore anatomy',
    'stage.tool.rotate': 'Rotate',
    'stage.tool.zoom': 'Zoom',
    'stage.tool.focus': 'Focus',
    'stage.tool.section': 'Cross-section',
    'stage.tool.layers': 'Layers',
    'stage.tool.compare': 'Compare',
    'stage.tool.reset': 'Reset',
    'stage.captionHint': 'Drag to rotate · Scroll to zoom in · Dots mark points of interest',
    'stage.lifeCue': 'See its {n} stages',
    'stage.lifeCueAria': 'Open the life cycle of {name} and step through its {n} stages on the display',
    'stage.error': 'The 3D model for this insect could not be built: {error}',
    'stage.loading': 'Building the 3D specimen of {name}…',
    'stage.noWebgl':
      'This device cannot show the 3D view (WebGL is unavailable), so here is the silhouette instead. The field-guide text is unaffected.',
    'stage.glLost':
      'The 3D view was interrupted (graphics context lost). It usually recovers by itself; reload if nothing happens.',
    'stage.reload': 'Reload',
    'stage.part.head': 'head',
    'stage.part.eye': 'compound eye',
    'stage.part.antenna': 'antenna',
    'stage.part.mouthparts': 'mouthparts',
    'stage.part.horn': 'horn',
    'stage.part.thorax': 'thorax',
    'stage.part.wing': 'wing',
    'stage.part.leg': 'leg',
    'stage.part.abdomen': 'abdomen',
    'stage.part.sound': 'sound organs',
    'stage.partJump': 'See another insect’s {part} →',
    'detail.eyebrow': 'Featured specimen',
    'detail.facts': 'Key facts',
    'detail.ecology': 'Ecological role',
    'detail.trivia': 'Did you know',
    'detail.readGuide': 'Read the full entry',
    'detail.motionDemo': 'Motion demo',
    'detail.quiz': 'Quiz',
    'detail.compareOthers': 'Compare with another insect',
    'detail.share': 'Share this insect',
    'detail.copyLink': 'Copy page link',
    'detail.linkCopied': 'Link copied',
    'detail.range': 'Range',
    'detail.status': 'Status',
    'detail.relatives': 'Relatives',
    'detail.photos': 'Photographs',
    'detail.photosLink': 'See real photos on iNaturalist',
  },
  {
    'stage.canvasLabel':
      "{name}ning interaktiv 3D namunasi: aylantirish uchun torting, kattalashtirish uchun aylantiring, tuzilishni ko'rish uchun rangli nuqtalarni bosing",
    'stage.tool.rotate': 'Aylantirish',
    'stage.tool.zoom': 'Kattalashtirish',
    'stage.tool.focus': 'Fokus',
    'stage.tool.section': 'Kesim',
    'stage.tool.layers': 'Qatlamlar',
    'stage.tool.compare': 'Taqqoslash',
    'stage.tool.reset': "Boshlang'ich holatga qaytarish",
    'stage.captionHint': "Aylantirish uchun torting · Yaqinlashtirish uchun aylantiring · Nuqtalar — kuzatuv joylari",
    'stage.lifeCue': '{n} ta bosqichini ko\'rish',
    'stage.lifeCueAria': "{name}ning hayot davrini oching va stendda {n} ta bosqichini birma-bir ko'ring",
    'stage.error': "Bu hasharotning modelini yaratib bo'lmadi: {error}",
    'stage.loading': "{name}ning 3D namunasi tayyorlanmoqda…",
    'stage.noWebgl':
      "Bu qurilmada 3D ko'rinishni ochib bo'lmaydi (WebGL mavjud emas), shuning uchun uning o'rniga siluet ko'rsatilmoqda. Jildning matn qismiga bu ta'sir qilmaydi.",
    'stage.glLost':
      "3D ko'rinish uzildi (grafik konteksti yo'qoldi). Odatda o'zi tiklanadi; agar tiklanmasa, sahifani qayta yuklang.",
    'stage.reload': 'Qayta yuklash',
    'stage.part.head': 'bosh',
    'stage.part.eye': "murakkab ko'z",
    'stage.part.antenna': 'mo\'ylov',
    'stage.part.mouthparts': "og'iz a'zolari",
    'stage.part.horn': 'shox',
    'stage.part.thorax': "ko'krak",
    'stage.part.wing': 'qanot',
    'stage.part.leg': 'oyoq',
    'stage.part.abdomen': 'qorin',
    'stage.part.sound': "ovoz va eshitish a'zolari",
    'stage.partJump': "Boshqa hasharotning {part}ini ko'rish →",
    'detail.eyebrow': 'Ushbu sonning namunasi',
    'detail.facts': "Asosiy ma'lumotlar",
    'detail.ecology': 'Ekologik roli',
    'detail.trivia': 'Bilasizmi',
    'detail.readGuide': "To'liq maqolani o'qish",
    'detail.motionDemo': 'Harakat namoyishi',
    'detail.quiz': 'Test',
    'detail.compareOthers': 'Boshqa hasharot bilan taqqoslash',
    'detail.share': 'Bu hasharotni ulashish',
    'detail.copyLink': 'Sahifa havolasini nusxalash',
    'detail.linkCopied': 'Nusxalandi',
    'detail.range': 'Tarqalishi',
    'detail.status': 'Holati',
    'detail.relatives': "Qarindosh turlari",
    'detail.photos': 'Haqiqiy fotosuratlar',
    'detail.photosLink': "iNaturalist saytida haqiqiy suratlarni ko'rish",
  },
)
